...
const imageMap = {
  ...
  recreation: "gym.png",
  server: "server.png",
  controll_room: "controll_room.png"
};

Object.keys(imageMap).forEach(type => {
  const file = imageMap[type];
  if (!file) return; // пропускаємо кабель
  ...
  img.addEventListener("error", () => {
    console.warn(`⚠️ Image not found: ${file}`);
  });
...
console.log("🧩 Loaded images:", Object.keys(itemImages));

Object.values(itemImages).forEach(img => {
  img.onload = () => console.log("✅ Loaded:", img.src);
  img.onerror = () => console.error("❌ Not found:", img.src);
});
...
const CATALOG = {
  ...
  space: { w: 160, h: 100, name: "Space", color: "#26de81", access: { x: 0, y: 100, w: 160, h: 60 } },
  monitor:    { w: 140, h: 60,  name: "Monitor", color: "#45aaf2", access: { x: 0, y: 60, w: 140, h: 40 } },
  
  // Exercise and recreation
  gym: { w: 220, h: 140, name: "Gym", color: "#fd79a8", access: { x: 0, y: 140, w: 220, h: 80 } },
  recreation: { w: 180, h: 120, name: "Recreation", color: "#fdcb6e", access: { x: 0, y: 120, w: 180, h: 60 } },
  
  // Privacy and psychology
  server: { w: 100, h: 80, name: "Server", color: "#6c5ce7", access: { x: 0, y: 80, w: 100, h: 40 } },
  controll_room: { w: 120, h: 60, name: "Controll Room", color: "#00b894", access: { x: 0, y: 60, w: 120, h: 40 } }
};
...
function drawObjects() {
  ...
  for (const o of objects) {
    ...
    const cat = CATALOG[o.type] || { color: "#666", name: o.type };
    ctx.strokeStyle = "#0b0b0b";
    ctx.lineWidth = 1.5;

    // Обчислення абсолютних координат на canvas
    const objX = activeModule.x + o.x;
    const objY = activeModule.y + o.y;

    // 1) фон (щоб картинка мала підкладку, якщо прозора)
    ctx.fillStyle = cat.color;
    ctx.fillRect(objX, objY, o.w, o.h);

    // 2) малюємо картинку якщо є
    const img = itemImages[o.type];
    if (img && img.complete && img.naturalWidth) {
      // Малюємо з невеликою внутрішньою відступом, якщо хочеш "рамку" навколо картинки — змініть padding
      // Тут малюємо повністю вміщеною в блок
      ctx.drawImage(img, objX, objY, o.w, o.h);
    } else {
      // fallback: якщо картинки немає / не завантажилась — залишаємо лише кольоровий блок
    }

    // 3) рамка об'єкта
    ctx.strokeRect(objX, objY, o.w, o.h);

    // 4) підпис у верхньому лівому кутку
    ctx.fillStyle = "#e6e6e6";
    ctx.font = "12px system-ui";
    ctx.textBaseline = "top";
    ctx.fillText(cat.name, objX + 6, objY + 6);

    // 5) підсвітка вибраного об'єкта
  ...
  // --- cable draft (залишено як у тебе) ---
  if (state.cableDraft) {
